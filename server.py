    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise RuntimeError(f"Failed to fetch FX rate from API: Received status code {response.status_code}")
...
@mcp.prompt()
def get_exchange_rate_prompt(currency: str, rate_type: str) -> str:
